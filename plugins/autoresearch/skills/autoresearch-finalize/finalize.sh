...
parse_groups() {
  ...
  # Serialize JSON fields into flat files so bash can consume them.
  DATA_DIR=$(mktemp -d)
  python3 -c "
import sys, json
with open('$groups_file') as f:
    config = json.load(f)
out = '$DATA_DIR'
open(out + '/base', 'w').write(config['base'])
open(out + '/trunk', 'w').write(config.get('trunk', 'main'))
open(out + '/final_tree', 'w').write(config['final_tree'])
open(out + '/goal', 'w').write(config['goal'])
open(out + '/count', 'w').write(str(len(config['groups'])))
for idx, group in enumerate(config['groups']):
    open(f'{out}/{idx}.title', 'w').write(group['title'])
    open(f'{out}/{idx}.body', 'w').write(group['body'])
    open(f'{out}/{idx}.last_commit', 'w').write(group['last_commit'])
    open(f'{out}/{idx}.slug', 'w').write(group['slug'])
...
print_summary() {
  ...
  echo "  rm -f autoresearch.jsonl autoresearch.sh autoresearch.md autoresearch.ideas.md autoresearch.checks.sh"
